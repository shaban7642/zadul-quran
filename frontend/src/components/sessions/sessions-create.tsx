...
const CreateSession = () => {
      ...
      try {
        ...
        if (isMounted()) {
          const filteredStudents: any[] = data.rows.filter(
            (row: any) => row?.role.name === "student"
          );
          const filteredTeachers: any[] = data.rows.filter(
            (row: any) => row?.role.name === "teacher"
          );
  ...
  const getSessions = useCallback(
    async (rowsPerPage: any, page: number) => {
      try {
        const data: any = await sessionApi.getSessions(rowsPerPage, page);
      ...
      <Box sx={{ width: "90%", m: "auto" }}>
        <FullCalendar
          ...
          headerToolbar={{
            ...
            center: "title",
            right:
              "addEventButton,multiMonthYear,listWeek,dayGridMonth,timeGridWeek,timeGridDay",
          }}
